...
try:
    ...
    cursor=con.cursor()
    cursor.execute("create table customer(name text, address text, mobile text, nationality text, aadhar integer, email text, gender text, father_name text)")
    con.commit()
    print("Table created") 
except:
...
cus_detail_frame = Frame(win, bg="#FFFFFF")
...
#Update button to search according to name nd update user's detail
# update_btn = Button(cus_detail_frame, text="Update", font=('Ariel', 10, 'bold'), bg='black', fg='yellow', width=8)
# update_btn.place(relx=.8, rely=.1)
...
def add(event):
    ...
    if (len(n) or len(ad) or len(mob) or len(nat) or len(aadhar) or len(email) or len(gender) or len(father)) == 0:
        ...
        try:
            con=sq.connect(database="database/main.sqlite")
            cursor=con.cursor()
            cursor.execute("insert into customer values(?,?,?,?,?,?,?,?)", (n,ad,mob,nat,aadhar,email,gender, father))
            con.commit()
            messagebox.showinfo("Success", "Details added")
        except:
            messagebox.showerror("Error", "Error occured. Try again after sometime")
...
def update(event):
    ...
    response = messagebox.askyesno("Update Details", "Update details for {}?".format(n))

    if response:
        try:
            con=sq.connect(database="database/main.sqlite")
            cursor=con.cursor()
            cursor.execute("UPDATE customer SET address=?, mobile=?, nationality=?, aadhar=?, email=?, father_name=? WHERE name=?", (ad,mob,nat, aadhar, email, father, n))
            con.commit()
            messagebox.showinfo("Updated", "Details updated!")
        except:
            messagebox.showerror("Error", "Error occured updating the record. Try again after sometime")
    else:
...
def delete(event):
    """"
    Event listner to delete the customer details on the basis of their name
    """
...
#Buttons
add_btn = Button(cus_detail_frame, text="Add", font=('Ariel', 12, 'bold'), bg='black', fg='yellow', width=12) 
add_btn.place(relx=.01, rely=.9)
add_btn.bind("<Button>", add) #Todo

update_btn = Button(cus_detail_frame, text="Update", font=('Ariel', 12, 'bold'), bg='black', fg='yellow', width=12) 
update_btn.place(relx=.25, rely=.9)
update_btn.bind("<Button>", update) #Todo

delete_btn = Button(cus_detail_frame, text="Delete", font=('Ariel', 12, 'bold'), bg='black', fg='yellow',  width=12) 
delete_btn.place(relx=.49, rely=.9)
delete_btn.bind("<Button>", delete) #Todo

reset_btn = Button(cus_detail_frame, text="Reset", font=('Ariel', 12, 'bold'), bg='black', fg='yellow',  width=12) 
...
search_view_frame = Frame(win, bg="yellow")
search_view_frame.place(relx=0.43, rely=.1, relwidth=.55,relheight=.85)

view_detail_lbl = Label(search_view_frame, text="View Detail and Search System", font=('Ariel', 18, "bold"))
view_detail_lbl.place(relx=.01, rely=.01)

search_by = Label(search_view_frame, text="Search By", font=('Ariel', 18, "bold"), bg='red', fg='white')
search_by.place(relx=.01, rely=.1)

search_combo = Combobox(search_view_frame, values=['Name', 'Mobile', "Email", "Aadhar"], font=('Ariel', 18, 'bold'))
...
search_btn = Button(search_view_frame, text="Search", font=('Ariel', 12, 'bold'), bg='black', fg='yellow',  width=12) 
search_btn.place(relx=.6, rely=.1)
search_btn.bind("<Button>", search) #Todo

showall_btn = Button(search_view_frame, text="Show all", font=('Ariel', 12, 'bold'), bg='black', fg='yellow',  width=12) 
showall_btn.place(relx=.75, rely=.1)
...
st = Text(search_view_frame,height=28,width=90, wrap="none", xscrollcommand=hori_scroll.set, font=("Arial", 12, "bold"))
...
# textVsb.config(command=st.yview)
# textHsb.config(command=st.xview)
# 
# textHsb.place(relx=.4, rely=.8)

# st = ScrolledText(search_view_frame, height=30, width=90)



# st.config(state='normal')
# st.xview(END)
# st.configure(yscrollcommand=textVsb.set, xscrollcommand=textHsb.set)
